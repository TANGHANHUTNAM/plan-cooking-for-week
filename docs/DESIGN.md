...
### Bổ sung 31/08/2026 — sort theo rating + ghi chú theo bữa

- **Sort trong tab Món ăn** (yêu cầu người dùng): select "Tên A→Z / Rating cao nhất" cạnh dãy filter chip (`foods-screen.tsx`, shadcn Select). Rating sort: `favoriteScore` giảm dần, đồng hạng theo tên (collation `vi`); client-side, kết hợp được với search + filter loại.
- **Ghi chú theo bữa** (yêu cầu người dùng — "lỡ buổi đó thiếu nguyên liệu gia vị"): cột mới `meals.note` (String?, migration `20260831040000_meal_note`), action `setMealNote(mealId, note)` — trim, rỗng = xóa, giới hạn 300 ký tự (hằng nội bộ vì file `"use server"` không export const được). UI trên `MealCard` cả 2 biến thể: nút "Thêm ghi chú" → textarea inline + Lưu/Hủy/Xóa ghi chú; có ghi chú thì hiện hộp vàng (amber) icon StickyNote, bấm vào để sửa. **Tab Đi chợ hiện ghi chú** dưới tiêu đề buổi (hộp vàng cùng kiểu) và **Copy kèm dòng `⚠ Ghi chú: …`** ngay dưới `▸ Tối nay (31/08)` (người dùng nhấn mạnh yêu cầu này); bữa có ghi chú vẫn hiện trong danh sách đi chợ kể cả khi món không có nguyên liệu cần mua.
- Verify 31/08: tsc/eslint sạch, 33 unit test pass, `next build` OK; browser: sort đổi thứ tự đúng (★5 → ★4 → ★3), thêm/sửa/xóa ghi chú trên Hôm nay + Lịch tuần, ghi chú hiện ở Đi chợ và trong clipboard khi Copy (hook `navigator.clipboard.writeText` để đọc), 0 lỗi console.
